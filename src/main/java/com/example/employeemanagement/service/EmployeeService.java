...
public class EmployeeService {
    ...
    public EmployeeResponse get(Long id) {
        return repository.findById(id).map(this::toResponse).orElseThrow(() -> new EmployeeNotFoundException(id));
    }
    ...
    public List<DepartmentStat> getDepartmentStats() {
        return repository.employeeCountByDepartment().stream().map(row -> new DepartmentStat(String.valueOf(row[0]), ((Number) row[1]).longValue())).toList();
    }
    ...
    private EmployeeResponse toResponse(Employee e) {
        return new EmployeeResponse(e.getId(), e.getName(), e.getDepartment(), e.getEmail(), e.getSalary(), e.isActive());
    }
